...
3. Preserve the current production contract: `npm run build` must produce the prebuilt output consumed by `npx nitro deploy --prebuilt`.
...
**Validation:** Run local dev/preview if possible, `npm run build`, inspect `.output/server/wrangler.json`, and run the GitHub deploy workflow on the branch or a controlled test before any main-branch release.
